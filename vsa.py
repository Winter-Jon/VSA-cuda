...
def generate_base_coords(h,w,ws):
    ...
    base_coords_h = torch.arange(ws) * 2 * ws / ws / (h-1)
    base_coords_h = (base_coords_h - base_coords_h.mean())
    base_coords_w = torch.arange(ws) * 2 * ws / ws / (w-1)
    base_coords_w = (base_coords_w - base_coords_w.mean())
...
def VSA_Attn(q, k, v, ws, attn_scale, sampling_offsets, sampling_scales, relative_position_bias):
